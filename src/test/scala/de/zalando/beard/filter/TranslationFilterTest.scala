...
class TranslationFilterTest extends FunSpec with Matchers{
  describe("Translation filter") {
    it("should  resolve locale and bundle when given correct arguments") {
      ...
      filter.apply("example.title", Map("bundle" -> "messages", "locale" -> "de")) shouldBe "Hallo"
      filter.apply("example.title", Map("bundle" -> "messages", "locale" -> "en")) shouldBe "Hello"
      filter.apply("example.title", Map("bundle" -> "messages", "locale" -> "it")) shouldBe "Ciao"
    }
